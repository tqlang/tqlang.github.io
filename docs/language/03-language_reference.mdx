...
Arrays (in tq, may also be called slices) are one of the most basic collection type in programming. It is based on the concept of a contiguous
...
Array lists are not implicitly allocated in memory. They need to be allocated with a predefined size and
cannot be automatically resized after creation. 

The array initialization syntax pre-allocates memory based on the length of the array which is
...
#### The Index Operator
To handle single values inside an array, it uses the index operator `[i]`.

The index operator is declared putting a value surrounded by square brackets after the array indentifier.
The declared value is the index, the position of the desired element on the list, starting from zero.
...
# Any numeric integer data type can be used
# as indexer!
let i8 myIndex = 0

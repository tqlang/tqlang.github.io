...
The tq language allows the user to choose between a long list
...
| `int`          | intptr_t        | (target dependent)  | (target dependent)              |
...
| `uint`         | uintptr_t       | (target dependent)  | (target dependent)              |

The `int` and `uint` types are special general-purpose integers that are defined
...
E.g.:
If compiling the project to an x86 architecture based machine, `int` will be equivalent
